...
在 2022 年年底，我主要不间断跟拍刘叔两个月，试图通过刘叔来了解这一群体，探讨关于孤独，老年社交，和公共空间的社会议题。`,
  ...
  },
  {
    ...
    year: "2024",
    description: "关于婚姻的隐喻性探索。试图通过影像呈现出婚姻中的隐喻性、平凡而真实的画面。",
  ...
  },
  {
    ...
    year: "2024",
    description: "纪实摄影作品系列。通过镜头观察城市角落里的模仿与荒诞，捕捉那些在刻意追求中流露出的真实生命状态。",
  ...
  },
  {
    ...
    year: "2023-2024",
    description: "个人频道的视觉探索与记录。",
    previewImg: "https://picui.ogmua.cn/s1/2026/02/24/699dbd73e4433.webp",
    content: [
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen relative overflow-x-hidden font-sans transition-colors duration-700 ${view === 'about' ? 'bg-[#0a0a0a] text-white' : 'bg-[#f0f0f0] text-black'}`}>
      ...
      {/* 胶囊导航栏 */}
      <nav className="fixed top-0 left-0 w-full z-[100] flex justify-center py-10 pointer-events-none">
        <div className="relative flex items-center pointer-events-auto">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setView(item.id)}
              className={`relative px-7 py-2 nav-link transition-colors duration-500 z-10 ${view === item.id ? 'text-white' : (view === 'about' ? 'text-white/40' : 'text-black/40')}`}
            >
              {item.label}
              {view === item.id && (
                <motion.div
      ...
      {/* 顶部标识 */}
      <div className="fixed top-0 left-0 w-full p-8 md:p-12 pointer-events-none z-[90] flex justify-between items-start">
        <div className="pointer-events-auto">
          <h1 className="text-[18px] font-black tracking-tighter leading-none cursor-pointer" onClick={() => setView('home')}>
            Liang Chujun
          </h1>
        </div>
        <a href="mailto:soulboyjade@qq.com" className="pointer-events-auto nav-link opacity-40 hover:opacity-100 transition-opacity">Contact</a>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {view === 'home' && (
          ...
          >
            <div className="h-screen w-full flex items-center justify-center">
              <motion.div className="relative w-[85vw] md:w-[60vw] aspect-[4/3] z-10 bg-neutral-200 overflow-hidden">
            ...
            <section className="w-full max-w-7xl px-8 md:px-12 py-20 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
              <h2 className="text-7xl md:text-[12vw] font-bold tracking-tighter leading-[0.8]">Gun*</h2>
              <div className="text-right font-sans text-sm md:text-xl leading-tight opacity-80">
                <p>Guangzhou / Shanghai,</p>
                <p>soulboyjade@qq.com</p>
              </div>
            </section>

            <section className="w-full max-w-7xl px-8 pb-40 text-left mx-auto">
              <div className="flex flex-col gap-8 items-start">
                <h3 className="text-4xl md:text-5xl font-bold sans-zh leading-tight">挑战</h3>
                <p className="text-lg md:text-xl opacity-70 leading-relaxed sans-zh font-light max-w-3xl">
                  影像的本质是捕捉那些转瞬即逝的真实感。在创作中，我们试图通过低调而有力的表达，让观众在静默中感受到情感的张力。
                </p>
              </div>
            </section>

            <section className="w-full max-w-6xl px-8 pb-60 border-t border-black/5 pt-20">
              <div className="flex flex-col">
                {PROJECTS.map((project) => (
                  <motion.div 
                    key={project.id}
                    className="group border-b border-black/5 py-10 cursor-pointer relative transition-colors duration-500 hover:bg-black/[0.03]"
                  ...
                  >
                    <div className="flex flex-col items-center justify-center text-center">
                      <h2 className="text-3xl md:text-4xl font-bold sans-zh tracking-tight transition-all duration-500 group-hover:italic">
                        {project.title}
                      </h2>
                    </div>
                  </motion.div>
        ...
        {view === 'work' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="min-h-screen pt-48 pb-60 px-6 md:px-8"
          >
          ...
          >
            <div className="flex flex-col gap-56">
              <section className="relative w-full h-screen overflow-hidden">
                ...
                <div className="relative h-full flex items-center px-8 md:px-20 max-w-7xl mx-auto">
                  <div className="flex flex-col gap-6 max-w-2xl">
                    <p className="text-lg md:text-2xl leading-snug sans-zh font-light text-white">
                      梁楚君<br/>
                      摄影师，影像创作者。
                    </p>
                    <p className="text-sm md:text-base opacity-60 sans-zh leading-relaxed text-white">
              ...
              <section className="px-8 max-w-5xl mx-auto w-full">
                <h3 className="text-2xl md:text-3xl font-black sans-zh leading-tight text-white">
                  致力于在日常琐碎中挖掘诗意，<br />
                  用影像记录时代的温度与人的尊严。
                </h3>
              </section>
              ...
              {/* 履历部分 */}
              <section className="flex flex-col px-8 max-w-5xl mx-auto w-full">
                <span className="nav-link text-white/30 uppercase tracking-[0.2em] mb-12">Experience / 履历</span>
                <div className="flex flex-col">
                  {EXPERIENCES.map((item, index) => (
                    <div 
                      key={index} 
                      className="group border-t border-white/20 py-10 cursor-pointer first:border-t-0"
                      onClick={() => setExpandedIndex(expandedIndex === index ? null : index)}
                    >
                      <div className="flex items-center justify-between gap-8">
                        {/* 字体修改：使用 font-light (细体) + sans-zh (黑体家族) */}
                        <h4 className="text-xl md:text-2xl font-light sans-zh group-hover:italic transition-all duration-500 text-white/90">
                          {item.title}
                        </h4>
      ...
      <AnimatePresence>
        {activeProject && (
          ...
          >
            <nav className="p-8 md:p-12 flex justify-between items-center sticky top-0 bg-[#f9f9f9]/80 backdrop-blur-md z-[2100]">
              <button onClick={() => setActiveProject(null)} className="flex items-center gap-2 group font-black text-[11px] tracking-widest uppercase text-black">
                <ArrowLeft size={16} /> <span>Back</span>
              </button>
            </nav>
            <div className="max-w-6xl mx-auto px-8 py-32">
              <h2 className="text-4xl md:text-[4vw] font-bold tracking-tight mb-8 sans-zh text-black">{activeProject.title}</h2>
              <div className="max-w-xl mb-32">
                <p className="text-lg leading-relaxed opacity-50 sans-zh text-black whitespace-pre-wrap">{activeProject.description}</p>
              </div>
              <div className="flex flex-col items-center gap-24 md:gap-40 mb-40">
      ...
      {/* 底部标识 */}
      <div className={`w-full p-8 md:p-12 flex justify-between items-end relative z-50 ${view === 'about' ? 'bg-[#0a0a0a] text-white border-t border-white/5' : 'border-t border-black/5'}`}>
        <h2 className="text-[18px] font-black tracking-widest sans-zh">梁 楚 君</h2>
        <div className="text-right font-sans text-[12px] leading-tight opacity-40">
          <p>Guangzhou / Shanghai,</p>
          <p>soulboyjade@qq.com</p>
        </div>
      </div>
